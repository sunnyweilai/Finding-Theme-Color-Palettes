...
def test_command_with_custom_description():
    def check(dirname):
        ...
        assert command.bokeh_app == 'lab_vif.py'
        assert command.description == 'hi'

    with_directory_contents_completing_project_file(
        {DEFAULT_PROJECT_FILENAME:
         "commands:\n default:\n    bokeh_app: lab_vif.py\n    description: hi\npackages:\n - bokeh\n"}, check)
...
def test_bokeh_command():
    def check_bokeh_command(dirname):
        ...
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(dirname, 'lab_vif.py'), '--show']
        assert cmd_exec.shell is False
...
def test_bokeh_command_with_extra_args():
    def check_bokeh_command_extra_args(dirname):
        ...
        path = os.pathsep.join([environ['PROJECT_DIR'], environ['PATH']])
        bokeh = _assert_find_executable('bokeh', path)
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(dirname, 'lab_vif.py'), '--show', '--foo']
        assert cmd_exec.shell is False
...
def test_bokeh_command_with_project_http_args():
    def check(dirname):
        ...
        path = os.pathsep.join([environ['PROJECT_DIR'], environ['PATH']])
        bokeh = _assert_find_executable('bokeh', path)
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(dirname, 'lab_vif.py'), '--address', '1.2.3.4', '--host',
...
def test_bokeh_command_with_multiple_host_args():
    def check(dirname):
        ...
        path = os.pathsep.join([environ['PROJECT_DIR'], environ['PATH']])
        bokeh = _assert_find_executable('bokeh', path)
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(dirname, 'lab_vif.py'), '--host', 'example.com', '--host',
                                 'example2.com', '--show']
        assert cmd_exec.shell is False
...
def test_bokeh_command_with_value_missing_for_project_http_args():
    def check(dirname):
        ...
        path = os.pathsep.join([environ['PROJECT_DIR'], environ['PATH']])
        bokeh = _assert_find_executable('bokeh', path)
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(dirname, 'lab_vif.py'), '--address', '', '--host', '',
...
def test_bokeh_command_with_disabled_project_http_args():
    def check(dirname):
        ...
        assert cmd_exec.args == [bokeh, 'serve', os.path.join(
            dirname, 'lab_vif.py'), '--foo', '--anaconda-project-url-prefix', 'blah', '--anaconda-project-port', '1234',
                                 '--anaconda-project-host', 'example.com', '--anaconda-project-no-browser',
                                 '--anaconda-project-use-xheaders']
        assert cmd_exec.shell is False
