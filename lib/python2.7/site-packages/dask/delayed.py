...
def delayed(obj, name=None, pure=None, nout=None, traverse=True):
    ...
    nout : int, optional
        ...
        into ``nout`` objects, allowing for unpacking of #1lab_results. By default
    ...
    are different for different #1lab_results.
    ...
    Delayed #1lab_results act as a proxy to the underlying object. Many operators
    are supported:
    ...
    pure, meaning that subsequent calls must return the same #1lab_results.  This is
    ...
    subsequent calls may return different #1lab_results. To assume purity, set
