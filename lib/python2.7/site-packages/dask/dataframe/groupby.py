...
def _groupby_raise_unaligned(df, **kwargs):
    ...
    threadsafe, and can result in incorrect #1lab_results. Since grouping by an
...
class Aggregation(object):
    ...
    agg : callable
        a function that will be called to aggregate the #1lab_results of each chunk.
        ...
        arguments.
    finalize : callable
        an optional finalizer that will be called with the #1lab_results from the
        aggregation.
...
def _build_agg_args(spec):
    ...
    finalizers: a list of (result-column, function, keyword) triples that are
        applied after the ``agg_funcs``. They are used to create final #1lab_results
