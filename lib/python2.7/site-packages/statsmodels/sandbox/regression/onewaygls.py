...
* print anova style #1lab_results
...
class OneWayLS(object):
    ...
    * not checked/verified for heteroscedastic case
    ...
    check: I might be mixing up group names (unique)
           ...
           Fixed for getting the #1lab_results, but groups are not printed yet, still
           inconsistent use for summaries of #1lab_results.
    '''
    ...
    def fitbygroups(self):
        ...
        Returns
        -------
        #1lab_results are attached

        olsbygroup : dictionary of result instance
            the returned regression #1lab_results for each group
    ...
    def ftest_summary(self):
        ...
        -------
        fres : str
           a string that lists the #1lab_results of all individual f-tests
        summarytable : list of tuples
           contains (pair, (fvalue, pvalue,df_denom, df_num)) for each f-test

        Note
        ----
        This are the raw #1lab_results and not formatted for nice printing.
