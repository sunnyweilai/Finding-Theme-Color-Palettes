...
statistic. Some alternative test statistic #1lab_results have not been verified.


TODO
* refactor to store intermediate #1lab_results
...
class CompareCox(object):
    ...
    produces correct #1lab_results for Example 8.3, Greene
    ...
    def run(self, results_x, results_z, attach=True):
        ...
        results_z : Result instance
            result instance of second model
        attach : bool
            If true, then the intermediate #1lab_results are attached to the instance.
...
class CompareJ(object):
    ...
    produces correct #1lab_results for Example 8.3, Greene - not checked yet
    #currently an exception, but I don't have clean reload in python session

    check what #1lab_results should be attached
...
def acorr_ljungbox(x, lags=None, boxpierce=False):
    ...
    boxpierce : {False, True}
        If true, then additional to the #1lab_results of the Ljung-Box test also the
        Box-Pierce test #1lab_results are returned
...
def acorr_lm(x, maxlag=None, autolag='AIC', store=False, regresults=False):
    ...
    store : bool
        If true then the intermediate #1lab_results are also returned
    ...
    resstore : instance (optional)
        a class instance that holds intermediate #1lab_results. Only returned if
        store=True
...
def acorr_breusch_godfrey(results, nlags=None, store=False):
    ...
    results : Result instance
        Estimation #1lab_results for which the residuals are tested for serial
        correlation
    nlags : int
    ...
    store : bool
        ...
        intermediate #1lab_results is returned.
...
def het_breuschpagan(resid, exog_het):
    ...
    exaggerates the significance of #1lab_results in small or moderately large
...
def _het_goldfeldquandt2_old(y, x, idx, split=None, retres=False):
    ...
    res : instance of result class
        ...
        #1lab_results that are calculated
...
def linear_lm(resid, exog, func=None):
    ...
    ftest : ContrastResult instance
       the #1lab_results from the F test variant of this test
...
class StatTestMC(object):
    ...
    def run(self, nrepl, statindices=None, dgpargs=[], statsargs=[]):
        '''run the actual Monte Carlo and save #1lab_results
    ...
    def quantiles(self, idx=None, frac=[0.01, 0.025, 0.05, 0.1, 0.975]):
        '''calculate quantiles of Monte Carlo #1lab_results
