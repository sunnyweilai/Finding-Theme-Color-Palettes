...
class StatTestMC(object):
    ...
    Attributes
    ----------
    many methods store intermediate #1lab_results

    self.mcres : ndarray (nrepl, nreturns) or (nrepl, len(statindices))
        Monte Carlo #1lab_results stored by run
    ...
    in the Monte Carlo #1lab_results.
    ...
    the #1lab_results should be presented, e.g.
    ...
    def run(self, nrepl, statindices=None, dgpargs=[], statsargs=[]):
        ...
        Returns
        -------
        None, all #1lab_results are attached
    ...
    def quantiles(self, idx=None, frac=[0.01, 0.025, 0.05, 0.1, 0.975]):
        ...
        idx : None or list of integers
            List of indices into the Monte Carlo #1lab_results (columns) that should
    ...
    def cdf(self, x, idx=None):
        '''calculate cumulative probabilities of Monte Carlo #1lab_results
...
#            ax = fig.addaxis()
    ...
    def summary_quantiles(self, idx, distppf, frac=[0.01, 0.025, 0.05, 0.1, 0.975],
        ...
        table : instance of SimpleTable
            use `print(table` to see #1lab_results
