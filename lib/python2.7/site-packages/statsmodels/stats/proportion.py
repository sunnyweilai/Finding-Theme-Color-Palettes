...
def power_ztost_prop(low, upp, nobs, p_alt, alpha=0.05, dist='norm',
    ...
    ``continuity`` and ``critval_continuity`` are added to match some #1lab_results
    ...
    **verification:**
    ...
    The ``dist='binom'`` #1lab_results match PASS,
    The ``dist='norm'`` #1lab_results look reasonable, but no benchmark is available.
...
def proportions_chisquare_allpairs(count, nobs, multitest_method='hs'):
    ...
    multitest_method : string
        ...
        that is used as default in the #1lab_results.
    ...
    result : AllPairsResults instance
        The returned #1lab_results instance has several statistics, such as p-values,
