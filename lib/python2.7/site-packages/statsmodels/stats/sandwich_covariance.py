...
def _get_sandwich_arrays(results, cov_type=''):
    """Helper function to get scores from #1lab_results
    ...
    if isinstance(results, tuple):
        ...
        raise ValueError('need either tuple of (jac, hessian_inv) or #1lab_results' +
                         'instance')
...
def _HCCM1(results, scale):
    ...
    results : result instance
       need to contain regression #1lab_results, uses #1lab_results.model.pinv_wexog
...
def _HCCM2(hessian_inv, scale):
    ...
    this uses #1lab_results.normalized_cov_params for (X'X)^(-1)

    Parameters
    ----------
    #1lab_results : result instance
       need to contain regression #1lab_results, uses #1lab_results.normalized_cov_params
...
def cov_cluster(results, group, use_correction=True):
    ...
    ----------
    results : result instance
       result of a regression, uses #1lab_results.model.exog and #1lab_results.resid
