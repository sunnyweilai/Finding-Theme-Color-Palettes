...
#        self.nobs2 = d2.sum_weights.astype(float)
    ...
    def summary(self, use_t=True, alpha=0.05, usevar='pooled', value=0):
        '''summarize the #1lab_results of the hypothesis test

        Parameters
        ----------
        use_t : bool, optional
            if use_t is True, then t test #1lab_results are returned
            if use_t is False, then z test #1lab_results are returned
        alpha : float
            significance level for the confidence interval, coverage is
            ``1-alpha``
