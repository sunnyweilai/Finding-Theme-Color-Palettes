...
class TestDiagnosticG(object):
    ...
    def test_influence(self):
        ...
        fp = open(os.path.join(cur_dir,"#1lab_results/influence_lsdiag_R.json"))
        lsdiag = json.load(fp)
        ...
        import pandas
        fn = os.path.join(cur_dir,"#1lab_results/influence_measures_R.csv")
        infl_r = pandas.read_csv(fn, index_col=0)
        conv = lambda s: 1 if s=='TRUE' else 0
        fn = os.path.join(cur_dir,"#1lab_results/influence_measures_bool_R.csv")
        #not used yet:
