...
def outlier_test(model_results, method='bonf', alpha=.05, labels=None,
    ...
    order : bool
        Whether or not to order the #1lab_results by the absolute value of the
...
class OLSInfluence(object):
    ...
    results : Regression Results instance
        currently assumes the #1lab_results are from an OLS regression

    Notes
    -----
    One part of the #1lab_results can be calculated without any auxiliary regression
    ...
    slower (mainly #1lab_results with `_external` postfix in the name).
    The auxiliary LOOO regression only the required #1lab_results are stored.
    ...
    def get_resid_studentized_external(self, sigma=None):
        ...
        sigma : None or float
            ...
            the estimate from the regression #1lab_results is used.
    ...
    def dffits_internal(self):
        ...
        uses original #1lab_results, no nobs loop
    ...
    def dffits(self):
        ...
        uses #1lab_results from leave-one-observation-out loop
    ...
    def _ols_xnoti(self, drop_idx, endog_idx='endog', store=True):
        '''regression #1lab_results from LOVO auxiliary regression with cache
    ...
    def _get_drop_vari(self, attributes):
        ...
        attributes : list of strings
           These are the names of the attributes of the auxiliary OLS #1lab_results
    ...
    @cache_readonly
    def _res_looo(self):
        '''collect required #1lab_results from the LOOO loop

        all #1lab_results will be attached.
    ...
    def summary_frame(self):
        """
        Creates a DataFrame with all available influence #1lab_results.

        Returns
        -------
        frame : DataFrame
            A DataFrame with all #1lab_results.
    ...
    def summary_table(self, float_fmt="%6.3f"):
        ...
        calculated from the original regression #1lab_results and for which a
        leave-one-observation-out loop is needed

        Returns
        -------
        res : SimpleTable instance
           SimpleTable instance with the #1lab_results, can be printed
...
def summary_table(res, alpha=0.05):
    ...
    st : SimpleTable instance
       table with #1lab_results that can be printed
    data : ndarray
