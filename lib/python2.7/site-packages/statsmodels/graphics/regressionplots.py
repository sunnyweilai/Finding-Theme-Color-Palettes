...
def plot_fit(results, exog_idx, y_true=None, ax=None, **kwargs):
    ...
    >>> fig, ax = plt.subplots()
    >>> fig = sm.graphics.plot_fit(#1lab_results, 0, ax=ax)
...
def _partial_regression(endog, exog_i, exog_others):
    ...
    Returns
    -------
    res1c : OLS #1lab_results instance

    (res1a, res1b) : tuple of OLS #1lab_results instances
         #1lab_results from regression of endog on exog_others and of exog_i on
         exog_others
...
def plot_ccpr(results, exog_idx, ax=None):
    ...
    ----------
    results : result instance
        A regression #1lab_results instance.
...
def ceres_resids(results, focus_exog, frac=0.66, cond_means=None):
    ...
    focus_exog : int
        The column of #1lab_results.model.exog used as the 'focus variable'.
    frac : float, optional
...
def added_variable_resids(results, focus_exog, resid_type=None,
    ...
    Parameters
    ----------
    results : regression #1lab_results instance
        A fitted model including the focus exog and all other
        predictors of interest.
    focus_exog : integer or string
        The column of #1lab_results.model.exog or a variable name that is
