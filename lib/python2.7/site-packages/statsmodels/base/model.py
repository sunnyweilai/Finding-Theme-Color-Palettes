        ...
        Optional arguments for solvers (see returned Results.mle_settings)::
            ...
            'basinhopping'
                ...
                T : float
                    ...
                    in function value will be accepted. For best #1lab_results
...
class GenericLikelihoodModel(LikelihoodModel):
    ...
    If the log-likelihood is specified for each observation, then #1lab_results that
    ...
    forward differentiation. This might #1lab_results in some cases in precision
...
class Results(object):
    """
    Class to contain model #1lab_results
...
class LikelihoodModelResults(Results):
    """
    Class to contain #1lab_results from likelihood models
    ...
    **Attributes**
    ...
    def t_test(self, r_matrix, cov_p=None, scale=None,
        ...
        res : ContrastResults instance
            The #1lab_results for the test are attributes of this #1lab_results instance.
            The available #1lab_results have the same elements as the parameter table
            in `summary()`.
        ...
        >>> data.exog = sm.add_constant(data.exog)
        >>> #1lab_results = sm.OLS(data.endog, data.exog).fit()
        >>> r = np.zeros_like(#1lab_results.params)
        >>> r[5:] = [1,-1]
        >>> print(r)
        [ 0.  0.  0.  0.  0.  1. -1.]
        ...
        >>> #1lab_results = ols(formula, dta).fit()
        >>> hypotheses = 'GNPDEFL = GNP, UNEMP = 2, YEAR/1829 = 1'
        >>> t_test = #1lab_results.t_test(hypotheses)
        >>> print(t_test)
                                     Test for Constraints
        ==============================================================================
                         coef    std err          t      P>|t|      [0.025      0.975]
    ...
    def f_test(self, r_matrix, cov_p=None, scale=1.0, invcov=None):
        ...
        >>> data.exog = sm.add_constant(data.exog)
        >>> #1lab_results = sm.OLS(data.endog, data.exog).fit()
        >>> A = np.identity(len(#1lab_results.params))
        >>> A = A[1:,:]

        This tests that each coefficient is jointly statistically
        significantly different from zero.

        >>> print(#1lab_results.f_test(A))
        ...
        >>> #1lab_results = ols(formula, dta).fit()
        >>> hypotheses = '(GNPDEFL = GNP), (UNEMP = 2), (YEAR/1829 = 1)'
        >>> f_test = #1lab_results.f_test(hypotheses)
        >>> print(f_test)
    ...
    def conf_int(self, alpha=.05, cols=None, method='default'):
        ...
        >>> #1lab_results.conf_int(cols=(2,3))
    ...
    def remove_data(self):
        ...
        #1lab_results and model instance.

        .. warning:: Since data and some intermediate #1lab_results have been removed
        ...
        model._data_attr : arrays attached to both the model instance
            and the #1lab_results instance with the same attribute name.
        ...
        result._data_attr_model : arrays attached to the model
            instance but not to the #1lab_results instance
        '''
        def wipe(obj, att):
...
class ResultMixin(object):
    ...
    def bootstrap(self, nrep=100, method='nm', disp=0, store=1):
        ...
        disp : bool
            If true, then optimization prints #1lab_results
        store : bool
...
class GenericLikelihoodModelResults(LikelihoodModelResults, ResultMixin):
    """
    A #1lab_results class for the discrete dependent variable models.
    ...
    mlefit : instance of LikelihoodResults
        This contains the numerical optimization #1lab_results as returned by
