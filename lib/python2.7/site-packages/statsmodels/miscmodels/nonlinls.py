...
class Results(object):
    ...
    most #1lab_results from RegressionResults can be used here
    '''
    pass
...
class NonlinearLS(Model):  #or subclass a model
    ...
    Warning:
    Weights are not correctly handled yet in the #1lab_results statistics,
    ...
    This is currently holding on to intermediate #1lab_results that are not necessary
    but useful for testing.

    Fit returns and instance of RegressionResult, in contrast to the linear
    model, #1lab_results in this case are based on a local approximation, essentially
    ...
    and use the non-linear regression #1lab_results, for example
