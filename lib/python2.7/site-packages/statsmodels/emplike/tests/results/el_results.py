...
class DescStatRes(object):
    ...
    The #1lab_results were generated from Bruce Hansen's
    MATLAb package:
    ...
    The R #1lab_results are from Mai Zhou's emplik package
...
class OriginResults(object):
    """
    These #1lab_results are from Bruce Hansen's Matlab package.
    To replicate the #1lab_results, the exogenous variables were scaled
    down by 10**-2 and the #1lab_results were then rescaled.
