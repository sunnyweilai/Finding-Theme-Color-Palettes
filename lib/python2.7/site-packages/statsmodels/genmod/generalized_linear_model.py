...
class GLM(base.LikelihoodModel):
    ...
    Using frequency weights: Frequency weights produce the same #1lab_results as repeating
    observations by the frequencies (if those are integers). This is verified for all
    basic #1lab_results with nonrobust or heteroscedasticity robust ``cov_type``. Other
...
class GLMResults(base.LikelihoodModelResults):
    """
    Class to contain GLM #1lab_results.
