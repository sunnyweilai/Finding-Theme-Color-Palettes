...
rfname = os.path.join(curdir,'#1lab_results','results_kde.csv')
#print rfname
KDEResults = np.genfromtxt(open(rfname, 'rb'), delimiter=",", names=True)

rfname = os.path.join(curdir,'#1lab_results','results_kde_univ_weights.csv')
KDEWResults = np.genfromtxt(open(rfname, 'rb'), delimiter=",", names=True)

# get results from R
curdir = os.path.dirname(os.path.abspath(__file__))
rfname = os.path.join(curdir,'#1lab_results','results_kcde.csv')
#print rfname
...
class TestKdeWeights(CheckKDE):
    ...
    def setupClass(cls):
        ...
        res1.fit(kernel="gau", gridsize=50, weights=weights, fft=False,
                    bw="silverman")
        cls.res1 = res1
        rfname = os.path.join(curdir,'#1lab_results','results_kde_weights.csv')
...
class TestKDEGaussFFT(CheckKDE):
    ...
    def setupClass(cls):
        ...
        cls.res1 = res1
        rfname2 = os.path.join(curdir,'#1lab_results','results_kde_fft.csv')
