...
class TestStatesAR3(object):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        path = current_path + os.sep + '#1lab_results/results_wpi1_ar3_stata.csv'
        cls.stata = pd.read_csv(path)
        cls.stata.index = pd.date_range(start='1960-01-01', periods=124,
                                         freq='QS')
        # Matlab comparison
        path = current_path + os.sep+'#1lab_results/results_wpi1_ar3_matlab_ssm.csv'
        matlab_names = [
        ...
        path = current_path + os.sep+'#1lab_results/results_wpi1_ar3_regression.csv'
...
class TestStatesMissingAR3(object):
    ...
    def setup_class(cls, *args, **kwargs):
        ...
        cls.stata.index = pd.date_range(start='1960-01-01', periods=124,
                                         freq='QS')
        # Matlab comparison
        path = current_path + os.sep+'#1lab_results/results_wpi1_missing_ar3_matlab_ssm.csv'
        matlab_names = [
            'a1','a2','a3','detP','alphahat1','alphahat2','alphahat3',
            'detV','eps','epsvar','eta','etavar'
        ]
        cls.matlab_ssm = pd.read_csv(path, header=None, names=matlab_names)
        # Regression tests data
        path = current_path + os.sep+'#1lab_results/results_wpi1_missing_ar3_regression.csv'
        cls.regression = pd.read_csv(path)
...
class TestMultivariateMissing(object):
    ...
    def setup_class(cls):
        # Results
        path = current_path + os.sep + '#1lab_results/results_smoothing_R.csv'
