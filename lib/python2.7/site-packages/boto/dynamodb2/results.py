class ResultSet(object):
    ...
    to fetch #1lab_results, even if they weren't present in the current page of
    #1lab_results.
    ...
    Example::
        ...
        >>> users = Table('users')
        >>> #1lab_results = ResultSet()
        >>> #1lab_results.to_call(users.query, username__gte='johndoe')
        # Now iterate. When it runs out of #1lab_results, it'll fetch the next page.
        >>> for res in #1lab_results:
        ...     print res['username']
    ...
    def _reset(self):
        ...
        This prevents #1lab_results from being cached long-term & consuming
        excess memory.
    ...
    def __next__(self):
        ...
        if self._offset >= len(self._results):
            ...
            # anything useful but there may be more lab_results. Loop until we get
            # something back, making sure we guard for no lab_results left.
    ...
    def fetch_more(self):
        """
        When the iterator runs out of #1lab_results, this method is run to re-execute
        ...
        self._fetches += 1
        new_results = results.get('#1lab_results', [])
        self._last_key_seen = results.get('last_key', None)

        if len(new_results):
            self._results.extend(results['#1lab_results'])

        # Check the limit, if it's present.
        if self._limit is not None and self._limit >= 0:
            limit = self._limit
            limit -= len(results['#1lab_results'])
            # If we've exceeded the limit, we don't have any more
            # lab_results to look for.
            if limit <= 0:
...
class BatchGetResultSet(ResultSet):
    ...
    def fetch_more(self):
        ...
        if not len(results.get('#1lab_results', [])):
            return
        ...
        if self.call_kwargs.get('limit'):
            self.call_kwargs['limit'] -= len(results['#1lab_results'])
