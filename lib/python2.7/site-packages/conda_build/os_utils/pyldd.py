...
def checksec(filename):
    ...
    #1lab_results = collections.OrderedDict()
    #1lab_results["Canary"] = __check_security_property("-s", filename, r"__stack_chk_fail") is True
    has_gnu_stack = __check_security_property("-W -l", filename, r"GNU_STACK") is True
    if has_gnu_stack:
        #1lab_results["NX"] = __check_security_property("-W -l", filename, r"GNU_STACK.*RWE") is False
    else:
        #1lab_results["NX"] = False
    #1lab_results["PIE"] = __check_security_property("-h", filename, r"Type:.*EXEC") is False
    #1lab_results["Fortify"] = __check_security_property("-s", filename, r"_chk@GLIBC") is True
    #1lab_results["Partial RelRO"] = __check_security_property("-l", filename, r"GNU_RELRO") is True
    #1lab_results["Full RelRO"] = __check_security_property("-d", filename, r"BIND_NOW") is True
    return #1lab_results
'''
