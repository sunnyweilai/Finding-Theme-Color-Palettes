...
class RSTStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, document, input_offset=0, match_titles=True,
        ...
        assert results == [], 'RSTStateMachine.run() #1lab_results should be empty!'
...
class NestedStateMachine(StateMachineWS):
    ...
    def run(self, input_lines, input_offset, memo, node, match_titles=True):
        ...
        self.node = node
        results = StateMachineWS.run(self, input_lines, input_offset)
        assert results == [], ('NestedStateMachine.run() #1lab_results should be '
                               'empty!')
        return results
...
class Body(RSTState):
    ...
    def parse_enumerator(self, match, expected_sequence=None):
        """
        Analyze an enumerator and return the #1lab_results.
