...
5. Run the state machine on the input text and collect the #1lab_results, a list::

       #1lab_results = sm.run(input_lines)
...
class StateMachine:
    ...
    #1lab_results of processing in a list.
    """
    ...
    def run(self, input_lines, input_offset=0, context=None,
        ...
        """
        Run the state machine on `input_lines`. Return #1lab_results (a list).
        ...
        Accumulate the #1lab_results returned by the transition methods in a list.
...
class State:
    ...
    values. `bof()` returns the initial context and #1lab_results, and may be used
