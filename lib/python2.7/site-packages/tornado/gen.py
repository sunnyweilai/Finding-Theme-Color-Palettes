...
class WaitIterator(object):
    """Provides an iterator to yield the #1lab_results of futures as they finish.

    Yielding a set of futures like this:

    ``#1lab_results = yield [future1, future2]``

    pauses the coroutine until both ``future1`` and ``future2``
    return, and then restarts the coroutine with the #1lab_results of both
    futures. If either future is an exception, the expression will
    raise that exception and all the #1lab_results will be lost.
    ...
    Because #1lab_results are returned as soon as they are available the
    ...
    def done(self):
        """Returns True if this iterator has no more #1lab_results."""
...
class WaitAll(YieldPoint):
    """Returns the #1lab_results of multiple previous `Callbacks <Callback>`.

    The argument is a sequence of `Callback` keys, and the result is
    a list of #1lab_results in the same order.
...
def multi(children, quiet_exceptions=()):
    ...
    #1lab_results. If ``children`` is a list, the result is a list of
    #1lab_results in the same order; if it is a dict, the result is a dict
    with the same keys.

    That is, ``#1lab_results = yield multi(list_of_futures)`` is equivalent
    to::

        #1lab_results = []
        for future in list_of_futures:
            #1lab_results.append(yield future)
...
class Runner(object):
    ...
    Maintains information about pending callbacks and their #1lab_results.

    The #1lab_results of the generator are stored in ``result_future`` (a
    `.TracebackFuture`)
    """
