...
def butter_lp(n, Wn):
    ...
    but it uses mpmath, and the #1lab_results are returned in lists instead of numpy
    arrays.
    """
    zeros = []
...
def zpkfreqz(z, p, k, worN=None):
    ...
    zpk format, the calculation is performed using mpath, and the #1lab_results are
