...
def place_poles(A, B, poles, method="YT", rtol=1e-3, maxiter=30):
    ...
    ``'KNV0'`` will yield #1lab_results similar to Matlab's or any other
    implementation of these algorithms.

    Using the default method ``'YT'`` should be fine in most cases; ``'KNV0'``
    is only provided because it is needed by ``'YT'`` in some specific cases.
    Furthermore ``'YT'`` gives on average more robust #1lab_results than ``'KNV0'``
    ...
    method while forcing 100 iterations of the algorithm and print some #1lab_results
    after each call.
    ...
    robustness of the #1lab_results, both ``'KNV0'`` and ``'YT'`` aim at maximizing
    it.  Below a comparison of the robustness of the #1lab_results above:
