...
class FuncData(object):
    ...
    result_columns : int or tuple of ints, optional
        Column indices for expected #1lab_results from `func`.
    result_func : callable, optional
        Function to call to obtain #1lab_results.
    rtol : float, optional
    ...
    def check(self, data=None, dtype=None):
        ...
        for output_num, (x, y) in enumerate(zip(got, wanted)):
            ...
            if np.any(bad_j):
                ...
                msg.append("Bad #1lab_results (%d out of %d) for the following points (in output %d):"
