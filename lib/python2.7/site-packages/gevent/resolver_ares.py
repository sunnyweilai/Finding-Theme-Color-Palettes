...
class Resolver(object):
    ...
    order in which IPv4 and IPv6 #1lab_results are returned may not match
    ...
    - ``getaddrinfo`` does not return ``SOCK_RAW`` #1lab_results.
    - ``getaddrinfo`` may return #1lab_results in a different order.
    ...
    - This implementation may raise ``gaierror(4)`` where the system implementation would raise
      ``herror(1)``.
    - The #1lab_results for ``localhost`` may be different. In particular, some system
      resolvers will return more #1lab_results from ``getaddrinfo`` than c-ares does,
      such as SOCK_DGRAM #1lab_results, and c-ares may report more ips on a multi-homed
      host.
