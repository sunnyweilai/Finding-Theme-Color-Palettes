...
_apply_docs = dict(
    template="""
    Apply function ``func``  group-wise and combine the #1lab_results together.

    The function passed to ``apply`` must take a {input} as its first
    argument and return a dataframe, a series or a scalar. ``apply`` will
    then take care of combining the #1lab_results back together into a single
    ...
    Calling ``apply`` in various ways, we can get different grouping #1lab_results:
...
* f must not mutate groups. Mutation is not supported and may
  produce unexpected #1lab_results.
