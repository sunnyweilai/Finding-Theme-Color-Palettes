...
def legmul(c1, c2):
    ...
    Notes
    -----
    In general, the (polynomial) product of two C-series #1lab_results in terms
    that are not in the Legendre polynomial basis set.  Thus, to express
    the product as a Legendre series, it is necessary to "reproject" the
    product onto said basis set, which may produce "unintuitive" (but
    correct) #1lab_results; see Examples section below.
...
def legdiv(c1, c2):
    ...
    #1lab_results in quotient and remainder terms that are not in the Legendre
    polynomial basis set.  Thus, to express these #1lab_results as a Legendre
    series, it is necessary to "reproject" the #1lab_results onto the Legendre
    basis set, which may produce "unintuitive" (but correct) #1lab_results; see
...
def leggauss(deg):
    ...
    The #1lab_results have only been tested up to degree 100, higher degrees may
    be problematic. The weights are determined by using the fact that

    .. math:: w_k = c / (L'_n(x_k) * L_{n-1}(x_k))

    where :math:`c` is a constant independent of :math:`k` and :math:`x_k`
    is the k'th root of :math:`L_n`, and then scaling the #1lab_results to get
