...
def chebmul(c1, c2):
    ...
    "unintuitive live" (but correct) #1lab_results; see Examples section below.
...
def chebdiv(c1, c2):
    ...
    #1lab_results in quotient and remainder terms that are not in the Chebyshev
    polynomial basis set.  Thus, to express these #1lab_results as C-series, it
    is typically necessary to "reproject" the #1lab_results onto said basis
    set, which typically produces "unintuitive" (but correct) #1lab_results;
