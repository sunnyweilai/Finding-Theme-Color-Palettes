...
def combine_dataframe(path_rgb, path_lab, output_path):
    ...
    for file in files:
        ...
        df.rename(columns=lambda x: str('rgb') + '_' + str(x[0]), inplace=True)
    ...
    for file in files1:
        ...
        df1.rename(columns=lambda x: str('lab') + '_' + str(x[0]), inplace=True)
...
path_rgb = 'rgb_allIQAs/rgb_results/rgb_50'
path_lab = 'lab_allIQAs/#1lab_results/lab_50'
output_path = 'combine_50'
